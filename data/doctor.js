...
module.exports = {
    ...
    async getDoctorByName(name){
        if (!name) 
            throw 'You must provide an name';
        const doctorCollection = await doctors();
        let res = await doctorCollection.findAll({ name: name});
        
        res["name"] = res.name; 
        if (res === null) 
            throw 'No doctor with that name';

        return res;
    },


    async getDoctorBySpec(spec){
        if (!spec) 
            throw 'You must provide an speciality';
        const doctorCollection = await doctors();
        ...
        for (i = 0; i < doctorspec.length; i++){
          ...
          if (doctorspec[i]['Speciality'] === spec){
            result.push(doctorspec[i])
          }
          if (doctorspec[i]['First_Name'] === spec){
            result.push(doctorspec[i])
          }
          if (doctorspec[i]['Last_Name'] === spec){
            result.push(doctorspec[i])
          }
        }
